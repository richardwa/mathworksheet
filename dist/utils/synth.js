...
exports.synth = (function (audioCtx) {
    var masterVolume = audioCtx.createGain(), currentNotes = [];
    function carrier(freq) {
        var gain = audioCtx.createGain(), osc = audioCtx.createOscillator();
    ...
    function fmod(node, multiplier, depth) {
        var gain = audioCtx.createGain(), osc = audioCtx.createOscillator(), freq = node.baseFreq;
    ...
    function adsr(node, ...phases) {
        var gain = node.gainSocket, param = gain.gain, now = audioCtx.currentTime;
    ...
    return {
        noteOn: function (id, midiKey, velocity, duration) {
            var hz = Math.floor(Math.pow(2, (midiKey - 69) / 12) * 440), level = velocity / 127, carry = carrier(hz), mod = fmod(carry, 2.1, .3), env = adsr(carry, p(10, level, false), p(500, 0, false));
